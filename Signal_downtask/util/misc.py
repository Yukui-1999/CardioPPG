...
def save_best_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler, test_stats, evaluation_criterion):
    ...
    remove_pattern = re.compile(r'^(events.*|.*\.log|log\.txt)$')
    file_names = [fname for fname in file_names if not remove_pattern.match(fname)]
    # save the 5 best performing models
    if len(file_names) > 3:
        
        file_names = sorted(file_names, key=lambda fname: int(re.search(r'\d+', fname).group()) if re.search(r'\d+', fname) else float('inf'))
    ...
    if loss_scaler is not None:
        checkpoint_paths = [output_dir / (f"checkpoint-{epoch_name}-{evaluation_criterion}-{test_stats[evaluation_criterion]:.3f}.pth")]
        for checkpoint_path in checkpoint_paths:
            to_save = {
                'model': model_without_ddp.state_dict(),
                'optimizer': optimizer.state_dict(),
