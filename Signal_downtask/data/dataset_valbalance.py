...
class MIMIC_Signal_dataset(data.Dataset):
    def __init__(self, data_index, args=None):
        ...
        elif args.label == '427_31':
            if self.train:
                self.target = np.load('/mnt/data2/ppg/dataset_all/processed_data/downtask_data/MIMIC/MIMIC_label_427_31_train.npy')
            else:
                self.target = np.load('/mnt/data2/ppg/dataset_all/processed_data/downtask_data/MIMIC/MIMIC_label_427_31_test.npy')
        elif args.label == '427_32':
            if self.train:
                self.target = np.load('/mnt/data2/ppg/dataset_all/processed_data/downtask_data/MIMIC/MIMIC_label_427_32_train.npy')
            else:
                self.target = np.load('/mnt/data2/ppg/dataset_all/processed_data/downtask_data/MIMIC/MIMIC_label_427_32_test.npy')
        ...
        self.ecg, self.signal, self.target = balance_data_all(self.ecg, self.signal, self.target)
        print(f'balanced')
        if self.train:
