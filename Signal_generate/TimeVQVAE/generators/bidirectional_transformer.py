...
class BidirectionalTransformer(nn.Module):
    ...
    def class_embedding(self, class_condition: Union[None, torch.Tensor], batch_size: int, device):
        ...
        if cond_type == 'uncond':
            class_uncondition = repeat(torch.Tensor([self.n_classes]).long().to(device), 'i -> b i', b=batch_size)  # (b 1)
            cls_emb = self.class_condition_emb(class_uncondition)  # (b 1 dim) (b 1 256)
            # print(f'cls_emb: {cls_emb.shape}')
            # exit()
            return cls_emb
        elif cond_type == 'class-cond':
            if self.training:
                ind = torch.rand(batch_size,class_condition.shape[-1]).to(device) > self.p_unconditional  # to enable classifier-free guidance
            else:
                ind = torch.ones_like(class_condition, dtype=torch.bool).to(device)
            ppg_cond = torch.where(ind, class_condition, torch.zeros_like(class_condition))  
            if self.kind == 'hf':
                ppg_cond = F.interpolate(ppg_cond.unsqueeze(1), size=2*ppg_cond.shape[-1], mode='linear', align_corners=False).squeeze(1)
                
            return ppg_cond.unsqueeze(1)
