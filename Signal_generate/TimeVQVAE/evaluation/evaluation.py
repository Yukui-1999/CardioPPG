...
class Evaluation(nn.Module):
    ...
    def __init__(self, 
        ...
        self.X_train = dataset_importer.X_train.astype(np.float64) 
        self.X_test = dataset_importer.X_test.astype(np.float64) 
        self.Y_train = dataset_importer.Y_train.astype(np.float64) 
        self.Y_test = dataset_importer.Y_test.astype(np.float64) 
        self.mean = dataset_importer.mean .astype(np.float64)  # scaling coefficient
        self.std = dataset_importer.std .astype(np.float64)  # scaling coefficient
        ...
        self.stage2 = ExpStage2.load_from_checkpoint(os.path.join('saved_models', f'stage2-{dataset_name}-2025-3-11-15-29-50.ckpt'), 
    ...
    def sample(self, n_samples: int, kind: str, class_index:Union[int,None]=None, unscale:bool=False, batch_size=None):
        ...
        print(f'x_new_l: {x_new_l.shape}, x_new_h: {x_new_h.shape}, x_new: {x_new.shape}, X_new_R: {X_new_R.shape}')
        # unscale
        if unscale:
    ...
    def compute_z(self, kind: str) -> np.ndarray:
        ...
        # get feature vectors from `X_test`
        zs = []
        for i in tqdm(range(n_iters), desc=f'compute_z ({kind})'):
            s = slice(i * self.batch_size, (i + 1) * self.batch_size)
    ...
    def compute_z_gen(self, X_gen: torch.Tensor) -> np.ndarray:
        ...
        z_gen = []
        for i in tqdm(range(n_iters), desc='compute_z_gen'):
            s = slice(i * self.batch_size, (i + 1) * self.batch_size)
    ...
    def inception_score(self, X_gen: torch.Tensor):
        ...
        p_yx_gen = []
        for i in tqdm(range(n_iters), desc='Inception Score'):
            s = slice(i * self.batch_size, (i + 1) * self.batch_size)
