...
@torch.no_grad()
def sample(batch_size:int, maskgit, device, n_samples: int, kind: str, class_index:Union[None,torch.Tensor]):
...
class Metrics(object):
    ...
    def compute_z_stat(self, x: np.ndarray) -> np.ndarray:
        ...
        # get feature vectors
        zs = []
        for i in tqdm(range(n_iters), desc='computing z test'):
    ...
    #     # get feature vectors
    #     zs = []
    #     for i in tqdm(range(n_iters), desc='computing z'):
    #         s = slice(i * self.batch_size, (i + 1) * self.batch_size)
    #         z = self.extract_feature_representations(x[s])
    #         zs.append(z)
    #     zs = np.concatenate(zs, axis=0)
    ...
    def compute_z(self, x: np.ndarray) -> np.ndarray:
        ...
        def process_batch(i):
            s = slice(i * self.batch_size, (i + 1) * self.batch_size)
            return self.extract_feature_representations(x[s])

        # 使用多进程并行计算
        zs = Parallel(n_jobs=cpu_count())(delayed(process_batch)(i) for i in tqdm(range(n_iters), desc='computing z'))
