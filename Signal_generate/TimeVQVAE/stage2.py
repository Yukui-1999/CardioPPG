...
def load_args():
    ...
    parser.add_argument('--config', type=str, help="Path to the config data  file.",
                        default=get_root_dir().joinpath('configs', 'config_ppgecg_stage1.yaml'))
    parser.add_argument('--dataset_names', nargs='+', help="e.g., Adiac Wafer Crop`.", default='')
    parser.add_argument('--gpu_device_ind', nargs='+', default=[0], type=int, help='Indices of GPU devices to use.')
    parser.add_argument('--feature_extractor_type', type=str, default='rocket', help='rocket | rocket for evaluation.supervised_fcn')
...
def train_stage2(config: dict,
    ...
    train_exp = ExpStage2(dataset_name, in_channels, input_length, config, n_classes, feature_extractor_type, use_custom_dataset, dataset_importer)

    n_trainable_params = sum(p.numel() for p in train_exp.parameters() if p.requires_grad)
    dt_now = datetime.datetime.now()
    now = "{}-{}-{}-{}-{}-{}".format(dt_now.year, dt_now.month, dt_now.day, dt_now.hour, dt_now.minute, dt_now.second)
    wandb_logger = WandbLogger(project=project_name, name=f'{now}', 
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=f'/mnt/sda1/dingzhengyao/Work/PPG_ECG/Signal_generate/timevq/stage2/{now}/',  # 指定保存路径
        every_n_train_steps=10000,     # 每隔 1000 步保存一次
        save_top_k=-1,                # 保存所有检查点
        filename='step_{step}',       # 文件名中包含步数
    )
    
    trainer = pl.Trainer(logger=wandb_logger,
                         callbacks=[LearningRateMonitor(logging_interval='step'), checkpoint_callback,ModelSummary(max_depth=5)],
    ...
    trainer.save_checkpoint(os.path.join(f'saved_models', f'stage2-{dataset_name}-{now}.ckpt'))
    ...
    (_, _, x_gen), _ = evaluation.sample(max(evaluation.X_test.shape[0], min_num_gen_samples), 'conditional')
    
    x_real = np.array(evaluation.X_test).squeeze()
    x_fake = 2 * normalize_per_sample(np.array(x_gen).squeeze()) - 1
    print(f'x_real.shape:{x_real.shape}, x_gen.shape:{x_fake.shape}')
    print(f'x_real.min:{x_real.min()}, x_real.max:{x_real.max()}')
    print(f'x_gen.min:{x_fake.min()}, x_gen.max:{x_fake.max()}')
    # RMSE MAE MSE
    rmse = np.sqrt(np.mean((x_real - x_fake) ** 2))
    mae = np.mean(np.abs(x_real - x_fake))
    mse = np.mean((x_real - x_fake) ** 2)
...
if __name__ == '__main__':
    ...
    for dataset_name in dataset_names:
        ...
        # train
        print(f'training stage2 for {dataset_name}...')
        train_stage2(config, dataset_name, train_data_loader, test_data_loader, args.gpu_device_ind, args.feature_extractor_type, args.use_custom_dataset,dataset_importer)
