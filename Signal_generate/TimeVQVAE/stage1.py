...
def load_args():
    ...
    parser.add_argument('--config', type=str, help="Path to the config data  file.",
                        default=get_root_dir().joinpath('configs', 'config_ppgecg.yaml'))
...
def train_stage1(config: dict,
    ...
    wandb_logger = WandbLogger(project=project_name, name=now, config={**config, 'dataset_name': dataset_name, 'n_trainable_params:': n_trainable_params})
    ...
    trainer.save_checkpoint(os.path.join(f'saved_models', f'stage1-{dataset_name}-{now}.ckpt'))
